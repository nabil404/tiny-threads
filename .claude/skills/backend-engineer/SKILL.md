...
- **A route's already-resolved host is trustworthy and worth reusing.** The middleware has confirmed it maps to a real tenant, so it's a sound basis for host-derived security checks — e.g. the OAuth `returnUrl` origin check in `apps/api/src/auth-core/return-url.ts` pins redirect targets to `req.hostname` rather than an allow-list, which keeps working when custom domains land.
