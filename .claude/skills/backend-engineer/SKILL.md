...
2. **Tenant context is set transaction-locally via `set_config(..., true)` inside `dataSource.transaction`, through the one central `withTenant` gate — never a bare `SET`, never string-interpolated.** ⚠️ And it is populated into CLS by exactly one thing, `TenantResolutionMiddleware`, from the request host — never from client-supplied input. ⚠️
...
- **`TenantResolutionMiddleware` (`apps/api/src/tenancy/tenant-resolution.middleware.ts`) is the concrete gate that populates CLS, and the only thing that does so** for ordinary requests. It resolves the tenant from the request's **subdomain** (`shop.platform.com` → slug `shop`), looks it up, `404`s on an unknown slug, and sets CLS `tenantId`. Everything below — `withTenant`, `TenantDbService.run`, every RLS policy — depends on it having run. (Custom-domain resolution is a known follow-up; JWT claims are **not** a tenant source here — see below.)
- ⚠️ **The `Host` header is client input — pin it to the platform's own domain before reading a slug out of it.** `req.hostname` is unvalidated by default. The middleware requires it to end with **`PLATFORM_HOST_SUFFIX`** (required env var, throws at boot if unset, leading dot enforced) and requires the remainder to be a single DNS label, `404`ing identically in every failure case so tenants can't be enumerated. Omit that check and `Host: shop.evil.example` resolves the **real** tenant `shop` — genuine slug, forged parent domain — an unauthenticated foothold on that tenant. The leading dot matters too: bare `platform.com` also tail-matches `evilplatform.com` → slug `evil`.
...
- **It is mounted `forRoutes('*')` in `AppModule` with a short, deliberate `.exclude(...)` list.** Adding an exclusion is a tenancy decision, not a routing tweak: an excluded route must either touch no tenant data or set CLS itself from a verified source. Current exclusions are `GET /auth/google/callback` (platform-domain route — Google allows one registered `redirect_uri`, so it sets CLS from the HMAC-signed OAuth state before any DB call) and `GET /` (liveness; health probes arrive by IP or internal DNS with no tenant slug, so behind the middleware every probe `404`s).
- **A route's host is reusable for security checks only downstream of that suffix validation** — never on its own. The OAuth `returnUrl` origin check in `apps/api/src/auth-core/return-url.ts` pins redirect targets to `req.hostname` rather than an allow-list (which keeps working when custom domains land), but it is sound *only* because the suffix check ran first: with a forged `Host`, both sides of that comparison are attacker-controlled and it passes trivially. Two rules follow — never use `req.hostname` as a security input on a route excluded from this middleware, and lowercase it before comparing (RFC 9110 makes `Host` case-insensitive, nothing normalizes it, and `URL.hostname` is already lowercased by the WHATWG parser, so a raw comparison spuriously rejects legitimate requests).
...
**Required tests:** for the gate itself, fire concurrent `withTenant` calls for different tenants through a small pool and assert each only ever sees its own rows — proves zero context bleed. For each new tenant-scoped table, prove **both** halves of the policy against a real database: a row is invisible under another tenant's context (`USING`), **and** a write stamped with another tenant's `tenant_id` is rejected (`WITH CHECK`). Assert the write case against the actual `violates row-level security policy` error (SQLSTATE `42501`), not merely that it threw — an incidental FK failure otherwise passes for RLS. Worked examples: `apps/api/test/customer-refresh-tokens-rls.e2e-spec.ts` and `merchant-user-refresh-tokens-rls.e2e-spec.ts`.
...
- **NotificationsPort** — ✅ **implemented** (the first port to land): `apps/api/src/auth-core/notifications/notifications-port.ts`, injected by the `NOTIFICATIONS_PORT` symbol, with `LogNotificationsAdapter` for dev/local. Shipped deliberately narrower than the design sketch below — `sendEmail(to, template, data)` over an `EmailTemplate` union (`verification-email`, `password-reset`, `merchant-invite`) — because auth is the only consumer so far and email the only channel; callers name an intent and never compose provider payloads. **Widen it (SMS/push channels, `idempotencyKey`, `parseEvent` for delivery/bounce) when a second consumer or channel actually needs it, not preemptively** — and when a real adapter (SES/SendGrid/Postmark) lands, it is one new adapter plus rebinding that token, with no domain change. Note for adapter authors: `LogNotificationsAdapter` redacts any `*token*` data key, because verification and reset tokens flow through `data` and log access alone must not be enough to hijack an account — keep that property in any adapter that logs.
