...
describe('CustomersAvatarService', () => {
  let service: CustomersAvatarService;
  ...
  beforeEach(() => {
    ...
    service = new CustomersAvatarService(
      tenantDbService,
      storagePort,
  ...
  describe('uploadAvatar', () => {
    it('processes image with Sharp, uploads to storage, updates DB row, and returns avatarUrl', async () => {
      const mockFile = {
        ...
        encoding: '7bit',
        mimetype: 'image/jpeg',
        buffer: Buffer.from('fake-customer-image-bytes'),
        size: 2048,
      ...
      storagePort.upload.mockResolvedValue({
        key: 'tenants/tenant-1/avatars/customers/cust-1.webp',
        url: 'http://storage.local/tenants/tenant-1/avatars/customers/cust-1.webp',
      });

      const customer = { id: 'cust-1', avatarUrl: null };
      ...
      const result = await service.uploadAvatar('cust-1', mockFile);
      ...
      expect(result).toEqual({
        avatarUrl:
          'http://storage.local/tenants/tenant-1/avatars/customers/cust-1.webp',
      });
    });
    ...
    it('throws FILE_TOO_LARGE when file exceeds 5MB limit', async () => {
      ...
      await expect(service.uploadAvatar('cust-1', mockFile)).rejects.toThrow(
        CodedBadRequestException,
      );
      try {
        await service.uploadAvatar('cust-1', mockFile);
      } catch (err: unknown) {
    ...
    it('throws INVALID_FILE_TYPE when file is missing or buffer is empty', async () => {
      ...
      try {
        await service.uploadAvatar('cust-1', emptyFile);
      } catch (err: unknown) {
        const res = (err as CodedBadRequestException).getResponse() as {
          code: string;
        };
  ...
  describe('deleteAvatar', () => {
    it('deletes file from storage and sets avatarUrl to null in DB', async () => {
      ...
      await service.deleteAvatar('cust-1');

      expect(storagePort.delete).toHaveBeenCalledWith(
        'tenants/tenant-1/avatars/customers/cust-1.webp',
      );
      expect(customer.avatarUrl).toBeNull();
    });

    it('does not attempt storage deletion if customer had no avatarUrl', async () => {
      const customer = { id: 'cust-1', avatarUrl: null };
    ...
    it('throws CodedUnauthorizedException if customer is not found', async () => {
      tenantDbService.run.mockImplementation(
        async (cb: (em: unknown) => Promise<unknown>) => {
      ...
      await expect(service.deleteAvatar('stale-customer')).rejects.toThrow(
        CodedUnauthorizedException,
      );
    });
  });
